...
func TestWireFieldsMatchSharedContract(t *testing.T) {
	...
	cases := []struct {
		...
		{"server:welcome", Welcome{T: "welcome", Ver: 1, Room: "ABC123", Me: "m1", Host: "m1",
			ShareAudio: true, Members: []Member{}, Timeline: tl, Waiting: []string{}, Now: 1}},
		{"server:timeline", NewTimelineMsg(tl)},
		{"server:members", MembersMsg{T: "members", Members: []Member{}, Host: "m1", ShareAudio: true, Waiting: []string{}}},
...
func TestValidateRejectsWhatARoomMustNeverSee(t *testing.T) {
	...
	tests := []struct {
		...
		{"mode with nothing to change", ClientMsg{T: "mode"}},
