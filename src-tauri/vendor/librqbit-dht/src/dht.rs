...
impl DhtWorker {
    ...
    async fn framer(
        ...
        let reader = async {
            ...
            loop {
                // A recv error is about one datagram, not about the socket: on
                // Windows an ICMP "port unreachable" from any node we sent to
                // comes back as WSAECONNRESET (os error 10054) on the next
                // recv_from, and a DHT talks to dead nodes constantly. Treat it
                // as noise (as the tracker comms do); only a socket that
                // returns nothing but errors is dead.
                let (size, addr) = match socket.recv_from(&mut buf).await {
                    ...
                    Err(e) => {
                        ...
                        if consecutive_errors > 100 {
                            return Err(e).context("too many consecutive UDP recv errors");
                        }
                        debug!("error reading from UDP socket (ignored): {e:#}");
                        continue;
                    }
                };
