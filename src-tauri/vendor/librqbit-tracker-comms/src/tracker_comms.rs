...
impl TrackerComms {
    ...
    async fn task_single_tracker_monitor_http(&self, mut tracker_url: Url) -> anyhow::Result<()> {
        ...
        // The announce URL may carry its own query (a passkey, rutracker's
        // `?magnet` marker). set_query below REPLACES the query, so keep the
        // original around and re-append it to every request — otherwise the
        // tracker refuses the announce (rutracker answers 403).
        let base_query = tracker_url.query().map(|q| q.to_string());
        loop {
            ...
            let mut request_query = request.as_querystring();
            if let Some(q) = &base_query {
                request_query.push('&');
                request_query.push_str(q);
            }
