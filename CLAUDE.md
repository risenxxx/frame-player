...
- **Three preferences are baked into the session and one comparison decides whether it can be reused.** Seeding, the port mapping and the proxy are all fixed when `Session::new_with_opts` runs, so changing any of them tears the … → [torrents-network](docs/rules/torrents-network.md)
...
- **Anything mpv keeps across a file change is a leak waiting to be noticed.** `video-rotate`, `video-aspect-override`, `panscan`, the A–B loop points and — measured — **`sub-delay` and `audio-delay`** all survive a `loadfile`, so … → [mpv-playback](docs/rules/mpv-playback.md)
